...
public class RegisterandSubmitProfile_Valid extends InstallandLaunchFlow {
  ...
  @Test
  public void register() throws EncryptedDocumentException, InvalidFormatException, IOException, InterruptedException {
	  
	  FileInputStream fis=new FileInputStream("C:\\Users\\gbsurao1\\eclipse-workspace\\FlowDrive\\src\\TestData\\testdata.xlsx");
	...
	while(!(driver.findElement(By.id("android:id/month")).findElement(By.className("android.widget.EditText")).getText()).equals(mm)) 
		{
			driver.findElement(By.name("Increase month")).click();
		}
	driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
	
	while(!(driver.findElement(By.id("android:id/day")).findElement(By.className("android.widget.EditText")).getText()).equals(dd)) 
		{
	...
	while(!(driver.findElement(By.id("android:id/year")).findElement(By.className("android.widget.EditText")).getText()).equals(yy)) 
		{ 
